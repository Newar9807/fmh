        ...
        <div id="signUpContaineer">
            <div id="signUpBoxId">
                <div id="signUpHeadingId">
                    <h3 style="border: none; margin-bottom: -11px;"> Fill the details below to sign up </h3>
                    <span style="border: none; font-size: bold;" id="signUpComment">
                        <?php
                ...
                <a href="signup.php"> Already have a account? </a>
            </div>
            ...
            <div id="emailVerificationBox">
                ...
                <p class="comment" id="verificationAlert"></p>
    ...
    <script>
        ...
        signUpFormSteps = (a, b) => {            
            if(a == 1){
                ...
                document.getElementById('verificationAlert').innerHTML = b;
            }
            else{
                ...
                document.getElementById('signUpComment').innerHTML = b;
            }
        }
...
<?php
    ...
    if((isset($_POST['btn'])) && (($_POST['email']) == "" || ($_POST['role']) == "" || ($_POST['name']) == "" || ($_POST['address']) == "" || ($_POST['dob']) == "" || ($_POST['gender']) == "" || ($_POST['phone']) == "" || ($_POST['password']) == "" || ($_POST['passwordConfirm']) == "" || ($_POST['marital_status']) == "")){
        $_SESSION['comment1'] = "<span class='fail'><center><b>Please fill up all boxes first</b></center></span>";              
        ?> <script> location.replace("http://localhost/findmehome/html/signup.php"); </script><?php
        return false;
    }
    elseif(isset($_POST['btn'])){
        if($_POST['password'] == $_POST['passwordConfirm']){
            ...
            $_SESSION['comment1']="<span class='fail'><center><b>Passwords are distinct</b></center></span>";
            ?> <script> location.replace("http://localhost/findmehome/html/signup.php"); </script><?php
            return false;
        }
        ...
        if($res == true){
            ...
            if($trigger){                        
                $_SESSION['comment1'] = "<span class='fail'><center><b>".$role." with this email already exists in database</b></center></span>";
                ?> <script> location.replace("http://localhost/findmehome/html/signup.php"); </script><?php
                return false;                
                // echo "end";
            }
                ...
                //Comparing given image extension with valid extension
                if(!in_array($file_check, $file_ext_stor)){
                    $_SESSION['comment1'] = "<span class='fail'><center><b>Image Extension is not matching.[png, jpg, jpeg]</b></center></span>";
                    selfCall();
                    return false;                  
                }
                elseif(!in_array($gfile_check, $gfile_ext_stor)){
                    $_SESSION['comment1'] = "<span class='fail'><center><b>Citezenship Extension is not matching.[png, jpg, jpeg]</b></center></span>";
                    selfCall();
                    return false;               
                }
                    ...
                    $_SESSION['ran'] = $ran;
                    // distroycomment1ie();
                    // setcomment1ie("email", $email, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("role", $role, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("name", $name, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("address", $address, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("dob", $date, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("gender", $gender, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("phone", $phone, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("password", $password, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("marital_status", $marital_status, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("note", $note, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;                    
                    // setcomment1ie("file", $file_name, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("gfile", $gfile_name, time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // setcomment1ie("count", "0", time() + (10 * 365 * 24 * 60 * 60), "signup.php") ;
                    // Query Successfull Executed                        
                    // $row = mysqli_fetch_assoc($res);
                    // while($row['email'])
                    // $to_email = $_POST['em'];
                    // setcomment1ie('ran', $ran, time() + (120), 'signup_core.php');
                    ...
                    if (mail($email, $subject, $body, $headers)) {
                        // $_SESSION['comment1'] = "<span class='success'><center>Verification code has been sent to '$email'</center></span>";//. mysqli_error();
                        ?> <script>
                            signUpFormSteps(1, "<span class='success'><center><b>Verification code has been sent to <?php echo $email;?></b></center></span>");
                        ...
                        $_SESSION['comment1'] = "<span class='fail'><center>Verification code cannot be sent to '$email'</center></span>";//. mysqli_error();
                        mysqli_close($conn);
                        selfCall();
                    }
    ...
    function selfCall(){
        ?> <script> 
            location.replace("http://localhost/findmehome/html/signup.php"); 
        </script><?php
    }
?>
...
<?php
    if(isset($_POST['verify'])){ 
        ...
        if(($_SESSION['ran']) == ($_POST['pin'])){           
            ...
            // $conn->query($sql);                    
            if($res == true){
                $_SESSION['comment1'] = "<span class='success'><center><b>Email Verified Successfully</b></center></span>";
                session_destroy();
                if($role == "Owner"){
                    $_SESSION['online'] = "Owner";
                    ?><script>
                        location.replace("http://localhost/findmehome/html/homeLandLord.php");
                    </script>
                    <?php
                }
                elseif($role == "Tenant"){
                    $_SESSION['online'] = "Tenant";
                    ?><script>
                ...
                // $_SESSION['comment1'] = "<span class='fail'><center><b>Error while interacting with database</b></center></span>";//. mysqli_error();
                // mysqli_close($conn);
                ?><script>
                    signUpFormSteps(1, "<span class='fail'><center><b>Error while interacting with database</b></center></span>");
                </script>
                <?php
            }
            return false;
            ...
            if($_SESSION['count'] == 3){
                session_destroy();
                // $_SESSION['comment1'] = "<span class='success'><center><b>You've Entered lots of invalid codes<br>All Sessions has been deleted</b></center></span>" ;//. mysqli_error();
                ?><script>
                    signUpFormSteps(2,"<span class='fail'><center><b>You've Entered lots of invalid codes</b></center></span>");
                </script>
                <?php
                return false;
            }
            ...
            ?><script>
                signUpFormSteps(1, "<span class='fail'><center><b>Verification code did not matched</b></center></span>");
            </script>
            <?php
            return false;
        }        
